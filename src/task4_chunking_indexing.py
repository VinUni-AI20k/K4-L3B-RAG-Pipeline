...
def _embed_openai(texts: list[str]) -> list[list[float]]:
    ...
    # Ép kiểu hoặc kiểm tra kĩ model ID truyền vào
    model = "text-embedding-3-small"
    
    # In ra console để kiểm tra tên model đang dùng
...
def _embed_gemini(texts: list[str]) -> list[list[float]]:
    ...
    all_embeddings: list[list[float]] = []
    # Gọi API cho từng chunk hoặc batch nhỏ để tránh lỗi payload/limit
    for text in texts:
